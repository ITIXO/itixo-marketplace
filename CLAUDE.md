...
- `changelog.md` is grouped by date, newest first, using `## YYYY-MM-DD` headings; each date is unique. By convention the file starts directly at the newest date heading with no H1 or preamble (not enforced by CI). Directly under each date, an optional common section of `-` bullets describes changes affecting multiple providers. Each release is `### <semver> - <provider>` with a plain ASCII hyphen; providers are `claude`, `codex`, `copilot`. No title text in the heading. Within a date, releases are grouped by provider alphabetically with versions strictly descending inside each provider; across the file, each provider's versions strictly descend in file order. A release body is optional and, when present, uses `-` bullets; a release with no body relies on the date's common bullets. Every date needs visible content and at least one release entry. Breaking changes are marked inline as `- **Breaking:** ...`. Every plugin version change must add `### <newversion> - <provider>` under some date heading; CI matches the changed plugin only against its own provider entries. New entries report only changes directly affecting plugin end users; omit repository or marketplace infrastructure, CI, build/release plumbing, generators, validation tooling, internal refactors, and any change without end-user impact. Wiki synchronization occurs automatically after a pull request.
