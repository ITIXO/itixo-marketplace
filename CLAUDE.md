...
Keep this table in sync with `base/rules/agents.md` and the shared model catalog at `plugins/codex/itixo/scripts/model-catalog.json`. Root aliases are global names with provider-specific `default` and `versions` entries; a provider entry means that alias is available there. Codex templates are inactive until explicitly installed; the Codex plugin's `itixo:install-agents` command reads the current catalog, filters to non-pinned Codex aliases, asks for personal vs project scope, then asks for each tier alias and the concrete version of each selected alias. Personal installs target `~/.codex/agents/`; project installs target `<project-root>/.codex/agents/` with an explicit root. Defaults are `luna` → GPT-6 Luna + high, `sol` → GPT-6 Sol + medium, and `astra` → GPT-6 Astra + max; choose `terra` → GPT-5.6 Terra + low when needed. Repeatable `--tier-model tier=alias` and `--model-version alias=concrete-id` options set the two layers independently; `--cheap-model` remains a compatibility shortcut for the cheap tier. Repeatable `--agent-model id=alias-or-concrete-id` and `--agent-effort id=none|low|medium|high|xhigh|max|ultra` options override any of the eight installed agents independently, with per-agent fields taking precedence. The installer only replaces exact Itixo-managed TOML files, refuses unmanaged conflicts, is idempotent for unchanged files, and requires a new task or Codex restart for discovery. Use `/itixo:update-models` from the source checkout to verify or promote model support; never edit an installed cache.

Claude keeps generated agent defaults unless the user explicitly requests a provider-supported model or effort override. The shared catalog's `opus` alias has a Claude provider entry; Claude Code v2.1.280+ resolves it to Opus 5.5 for Anthropic, API, AWS Bedrock, and Google Vertex, while Foundry resolves an older Opus version. See the [official Claude model configuration docs](https://code.claude.com/docs/en/model-config) for provider-specific aliases and restrictions. Pin older Claude choices through the provider's documented model configuration or environment settings. Provider or organization restrictions can still constrain either provider.
...
- Three per-provider changelog files at repo root: `changelog.claude.md`, `changelog.codex.md`, `changelog.copilot.md`; each file has no provider heading, preamble, or cross-provider common section. Format per file: `## YYYY-MM-DD` date headings (strictly descending, each date unique) with no preamble before the first date, then nested `### <plugin_name> MAJOR.MINOR.PATCH` headings (strictly descending per plugin across the whole file) with `-` bullet bodies; every date requires at least one plugin heading, and every plugin heading requires at least one `- ` bullet line. A change affecting multiple providers must bump every affected provider's plugin version on the same date, with the (duplicated) bullet text written into each affected provider's file under that date's plugin/version heading. Breaking changes are marked inline as `- **Breaking:** ...`. Every plugin version change must add `### <plugin_name> <newversion>` under the appropriate date in the provider's file; CI validation runs `node scripts/validate-plugin-changes.js --base <commit> --changelog-dir .`, which validates each file independently, dynamically discovers changed plugins from provider/plugin paths, and requires matching plugin/version headings in the corresponding provider file, including new plugins. New entries report only changes directly affecting plugin end users; omit repository or marketplace infrastructure, CI, build/release plumbing, generators, validation tooling, internal refactors, and any change without end-user impact. Wiki synchronization occurs automatically after a pull request, composing the wiki's `Changelog.md` without a `# Changelog` heading, then `# claude`, `# codex`, and `# copilot` headings in that fixed order, with each provider's file content verbatim (`##` dates and `###` plugin/version headings).
