...
plugins/turborepo-claude/         # Turborepo plugin for Claude (turborepo-mfe, add-app, add-package skills)
plugins/turborepo-codex/          # Turborepo plugin for Codex (turborepo-mfe, add-app, add-package skills)
plugins/turborepo-copilot/        # Turborepo plugin for Copilot (turborepo-mfe, add-app, add-package skills)
...
1. Create `plugins/<plugin>-<provider>/.claude-plugin/plugin.json` (Claude) and/or `plugins/<plugin>-<provider>/.codex-plugin/plugin.json` (Codex). Every plugin directory is named `<plugin>-<provider>` with the provider last (`itixo-claude`, `turborepo-codex`); the plugin name itself may contain dashes but must not end in a provider name.
...
Provider identity is hardcoded in `scripts/providers.js`, shared by `scripts/generate-agents.js` and `scripts/validate-plugin-changes.js`; introducing a new provider requires updating `PROVIDERS` in `scripts/providers.js`, which also derives the `<plugin>-<provider>` directory pattern. A `plugins/<dir>` whose trailing segment is not a listed provider fails validation with a hard error.
...
- `changelog.md` is grouped by date, newest first, using `## YYYY-MM-DD` headings; each date is unique and strictly descends. By convention the file starts directly at the newest date heading with no H1 or preamble (not enforced by CI). Directly under each date is an optional common section of `-` bullets describing changes affecting multiple plugins or providers. Under each date, providers are listed as `### <provider>` (`claude`, `codex`, `copilot`), alphabetically ordered and appearing at most once per date, with no content before the first plugin heading. Under each provider, plugins are listed as `#### <plugin>` — the bare plugin name without the provider suffix (`itixo`, `turborepo`) — alphabetically ordered, at most once per provider section, with no content before their first version. A `### <provider>` plus `#### <plugin>` pair identifies the `plugins/<plugin>-<provider>` directory, and each such directory owns exactly one version line. Under each plugin are versions as `##### <semver>` with no title text; versions strictly descend under their plugin directory across the whole file. A version body is optional and, when present, uses `-` bullets; a bodyless version relies on the date's common bullets. Every date needs visible content and at least one provider, every provider at least one plugin, every plugin at least one version. Breaking changes are marked inline as `- **Breaking:** ...`. Every plugin version change must add `##### <newversion>` under its `#### <plugin>` and `### <provider>` headings beneath some date heading; CI matches the changed plugin only against its own directory's versions. New entries report only changes directly affecting plugin end users; omit repository or marketplace infrastructure, CI, build/release plumbing, generators, validation tooling, internal refactors, and any change without end-user impact. Wiki synchronization occurs automatically after a pull request.
