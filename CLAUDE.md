...
plugins/itixo-component-library-claude/    # Component-library plugin for Claude (itixo-component-library skill)
plugins/itixo-component-library-codex/     # Component-library plugin for Codex (itixo-component-library skill)
plugins/itixo-component-library-copilot/   # Component-library plugin for Copilot (itixo-component-library skill)
...
- `changelog.md` is grouped by date, newest first, using `## YYYY-MM-DD` headings; each date is unique and strictly descends. By convention the file starts directly at the newest date heading with no H1 or preamble (not enforced by CI). Directly under each date is an optional common section of `-` bullets describing changes affecting multiple plugins or providers. Plugins are listed as `### <plugin-directory>` (bare `plugins/` directory name: `itixo-claude`, `itixo-component-library-codex`, …), alphabetically ordered within a date, appearing at most once per date. Each `plugins/<dir>` owns exactly one version line, so the directory — not the bare provider — is what a version heading belongs to. Under each plugin are versions as `#### <semver>` with no title text; versions strictly descend under their plugin and across the file per plugin; plugin headings have no content before their first version. A version body is optional and, when present, uses `-` bullets; a bodyless version relies on the date's common bullets. Every date needs visible content and at least one plugin, every plugin at least one version. Breaking changes are marked inline as `- **Breaking:** ...`. Every plugin version change must add `#### <newversion>` under `### <plugin-directory>` beneath some date heading; CI matches the changed plugin only against its own directory's versions. New entries report only changes directly affecting plugin end users; omit repository or marketplace infrastructure, CI, build/release plumbing, generators, validation tooling, internal refactors, and any change without end-user impact. Wiki synchronization occurs automatically after a pull request.
