...
The package publishes to a private GitHub Packages NuGet feed under `ITIXO-Playground`, alongside `nuget.org`. Consumers need a `nuget.config` with package source mapping, not just the default `nuget.org` source, or `dotnet restore` will fail to find `Itixo.*` packages.
...
<configuration>
    <packageSources>
        ...
        <add key="github" value="https://nuget.pkg.github.com/ITIXO-Playground/index.json" />
    </packageSources>
