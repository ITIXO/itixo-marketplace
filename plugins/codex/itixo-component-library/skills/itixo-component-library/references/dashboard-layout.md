...
> - **Never set `allowCustomTypography: true` on your own.** It is a backward-compatibility escape hatch for apps mid-migration: it disables the type-scale guard app-wide and produces inconsistent font sizes across ITIXO products. If a task appears to need it, stop and ask the developer to confirm they really want to break the shared typography, and offer the token-based fix (`<Typography>` / `typographyVariants()`) first.
