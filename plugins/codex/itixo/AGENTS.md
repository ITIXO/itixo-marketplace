...
For every delegation, invoke the installed custom TOML agent by its canonical ID. Do not load `plugins/codex/itixo/agents/*.md` or substitute a generic agent. The installed TOML owns its instructions, model, and effort. If the user explicitly requests a per-agent override, relay it to `itixo:install-agents`; do not infer values, broaden it to another agent, or pass a separate invocation override. An explicitly installed planner GPT-6 Sol override may exceed the caller model, subject to provider or organization restrictions.

If a required `itixo-*` custom agent is unavailable or its requested override is not installed, stop the affected work. Tell the user installation is required; do not perform that role inline. Invoke or offer `itixo:install-agents`, and obtain its explicit scope, tier-alias, model-version, effort, and optional per-agent choices before installation. Recommend `luna` → GPT-6 Luna + high, `sol` → GPT-6 Sol + medium, and `astra` → GPT-6 Astra + max; `terra` → GPT-5.6 Terra + low is the fallback.

The packaged Codex catalog is shared model data for all providers. Root aliases are available only where their provider entry exists; Codex installation filters to non-pinned Codex entries, then prompts for the selected alias and its Codex-specific version.
