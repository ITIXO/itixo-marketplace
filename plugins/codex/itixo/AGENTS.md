...
If a required `itixo-*` custom agent is unavailable or its requested override is not installed, stop the affected work. Tell the user installation is required; do not perform that role inline. Invoke or offer `itixo:install-agents`, and obtain its explicit scope, cheap-model, cheap-effort, and optional per-agent choices before installation. Recommend GPT-6 Luna + high; GPT-5.6 Terra + low is the fallback.
